...
import { Award, Plus, Trash2, Trophy, Globe, Heart } from 'lucide-react';
...
export const AchievementsForm = () => {
  ...
  const change = (i, val) => { const c = [...achievements]; c[i].text = val; updateSection('achievements', c); };
      ...
      <AnimatePresence>
        {achievements.map((ach, i) => (
          <motion.div key={ach.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <Trophy size={14} className="text-[var(--olive-drab)] mt-3 shrink-0" />
            ...
            <button onClick={() => remove(i)} className="p-2 mt-0.5 text-red-400 hover:text-red-600 transition-colors">
              <Trash2 size={13} />
            </button>
          </motion.div>
        ))}
...
export const InterestsForm = () => {
  ...
  const [useState] = [require => {
    // This is a workaround — interests form uses local state
  }];
...
export const InterestsFormFixed = () => {
  const { resumeData, updateSection } = useResumeStore();
  const interests = resumeData.interests || [];
  const [input, setInput] = useState('');
  ...
  const suggestions = ['Open Source', 'Machine Learning', 'Photography', 'Travel', 'Gaming', 'Music', 'Reading', 'Fitness'];
